...
# Directly copied from (and modified against) viesturz/klipper-toolchanger's
# tools_calibrate.py: https://github.com/viesturz/klipper-toolchanger/blob/main/klipper/extras/tools_calibrate.py
# Modified 2026-08-14 for BBox Toolchanger: dropped TOOL_CALIBRATE_SAVE_TOOL_OFFSET
# (saved to printer.cfg; BBox persists offsets to variables.cfg via
# BT_SAVE_TOOL_OFFSET instead), and added a gcode_move.reset_last_position()
# call in locate_sensor() to fix stale toolhead-position tracking.
...
class ToolsCalibrate:
    ...
    def locate_sensor(self, gcmd):
        ...
        # manual_move()/set_position() reposition the toolhead without telling
        # gcode_move, so its tracked "last position" goes stale here. Resync it
        # now so anything reading printer.gcode_move.gcode_position right after
        # this command (e.g. BBox's toolchange saved_x/y/z capture) sees the
        # real parked position instead of wherever the last real G1/G0 left off.
